...
/* ---------------------------------------------------------------------------
 * Phase 1-9 DEV HARNESS (Gate A0/A/B/C/M + Gate P probes), spec §22/§23.4.
 *
 * This whole module is the no-leak surface the shipped bundle must not carry
 * (spec §18 "Debug tooling gated behind an explicit flag"; plan graybox policy
 * "debug tooling off by default"). It is reached ONLY through the harness-gated
 * dynamic import in `src/main.ts` (`if (__VR_HARNESS__) import('./dev/harness')`):
 * a production `vite build` constant-folds `__VR_HARNESS__` to `false`, so this
 * module and every `__vr/__dbg/__probe/…` global, the pose registry, the kit
 * boards, the gray display pads and the query-driven material/culling levers
 * are dead-code-eliminated out of the shipped asset (Phase 10 no-leak contract).
 *
 * It backs the deterministic Playwright drivers `scripts/shots.mjs`,
 * `scripts/probe.mjs` and `scripts/perf.mjs`, which build this variant via
 * `npm run build:harness` and drive it through `window.__vr` / `window.__dbg`.
 * None of it affects play: the real game is `game/Game.ts`.
 * ------------------------------------------------------------------------- */
...
export function installDevHarness(game: Game, canvas: HTMLCanvasElement, buildMs: number): void {
  ...
  /* bind the deterministic shot API onto this live game (spec §22). The rAF loop
     is handed back to the live sim through the host's releasePose hook, which
     replaces the old Game-side monkey-patch. */
  Debug.bind({
    applyPose: (p) => game.applyPose(p),
    render: (m) => view.render(m),
    getCanvas: () => canvas,
    releasePose: () => game.releasePose(),
  })

  /* query-driven debug material/culling levers (dev-only; forbidden by default) */
  ...
  ;(globalThis as unknown as { __probe?: () => object }).__probe = () => ({
    ...
    err: !((document.getElementById('error-screen')) as HTMLElement)?.classList.contains('hidden') ? ((document.getElementById('error-body')) as HTMLElement)?.textContent?.slice(0, 600) ?? '' : null,
